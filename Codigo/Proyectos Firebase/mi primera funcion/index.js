...
app.intent('adivina el numero', (conv) => {
  conv.data.numeroOculto = Math.floor((Math.random() * 9) + 1);
...
app.intent('obtener número', (conv, {numero}) => {
  ...
  numeroOculto = conv.data.numeroOculto;
  ...
  if(numero == numeroOculto)  conv.close('Enhorabuena, ha acertado');
  		    ...
  		    else conv.ask('El número en el que he pensado es mayor');
});

app.intent('sigue la secuencia', (conv) => {
  let secuencia = generarSecuencia();
  conv.data.secuenciaNumeros = secuencia;
  conv.ask("Repita los siguientes números: " + secuencia);
});
...
function generarSecuencia(){
  let secuencia = "";
  for(var i = 0; i < 5; i++){
    secuencia = secuencia + Math.floor((Math.random() * 9)) + " ";
  }
...
exports.obtenerFechaHoraActual = functions.https.onRequest((req, res) => {
  var fechaHoraActual = new Date();
  res.send(fechaHoraActual.toString());
});
